pipeline {
	...
	stages {
		...
		stage('PR previews') {
			...
			steps {
				container(CONTAINER_ID) {
					...
					sh 'mvn -DargLine=\'-Dspring.profiles.active=test,test-kube,insecure,in-rest,out-rest\' install'
		...
		stage('Contract tests') {
			...
			steps {
				container(CONTAINER_ID) {
					sh "mvn -Dtest=*ContractTest -DargLine='-Dspring.profiles.active=test,test-kube,insecure' test"
				}
			}
		}
		...
		stage('Static code analysis') {
			...
			steps {
				container(CONTAINER_ID) {
					...
					// TODO to be replace by more modern SpotBugs
					// sh 'mvn findbugs:findbugs xslt:transform findbugs:check'
				}
			}
		}
		...
		stage('Create repository tag & release') {
			when {
				expression {
					!VERSION_ALIGN_COMMIT && !IS_PR && !IS_FEATURE_BRANCH
				}
